...
const tokenCosts = [
  ...
  { action: 'Reanálisis manual',             cost: '0.02',  desc: 'Actualiza el score cuando quieras', icon: 'M1 4v6h6 M23 20v-6h-6 M20.49 9A9 9 0 005.64 5.64L1 10 M3.51 15a9 9 0 0014.85 3.36L23 14',            color: '#475569', bg: '#f1f5f9', border: '#e2e8f0' },
]
...
function FaqItem({ q, a, defaultOpen = false }: { q: string; a: string; defaultOpen?: boolean }) {
  ...
  return (
    <div style={{ borderRadius: 16, border: `1.5px solid ${open ? '#bfdbfe' : '#f1f5f9'}`, background: open ? '#f8fbff' : '#fff', overflow: 'hidden', transition: 'border-color 0.2s, background 0.2s', boxShadow: open ? '0 4px 20px rgba(93,167,236,0.08)' : '0 2px 8px rgba(0,0,0,0.03)' }}>
      <button onClick={() => setOpen(v => !v)} style={{ width: '100%', textAlign: 'left', padding: '18px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 14, background: 'transparent', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>
        ...
        <div style={{ width: 28, height: 28, borderRadius: '50%', flexShrink: 0, background: open ? '#5da7ec' : '#f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.2s, transform 0.3s', transform: open ? 'rotate(45deg)' : 'rotate(0deg)' }}>
    ...
    <>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @keyframes pr-check-in { 0%{stroke-dashoffset:20;opacity:0} 60%{opacity:1} 100%{stroke-dashoffset:0} }
        .pr-check-icon { stroke-dasharray:20; stroke-dashoffset:20; animation:pr-check-in 0.4s 0.1s cubic-bezier(0.16,1,0.3,1) forwards; }
        ...
        input[type=range]::-webkit-slider-thumb { -webkit-appearance:none; width:20px; height:20px; border-radius:50%; background:#5da7ec; cursor:pointer; box-shadow:0 2px 8px rgba(93,167,236,0.4); border:2px solid #fff; }
        @media(max-width:960px) { .pr-plans-grid { grid-template-columns:1fr!important; max-width:480px!important; margin:0 auto!important; } .pr-guar-grid { grid-template-columns:1fr 1fr!important; } }
        @media(max-width:680px) { .pr-hero-btns { flex-direction:column!important; align-items:center!important; } }
        @media(max-width:440px) { .pr-guar-grid { grid-template-columns:1fr!important; } }
      `}</style>

      {/* ── HERO ── */}
      <section style={{ background: 'linear-gradient(140deg,#050d1f 0%,#0c1e42 55%,#071428 100%)', padding: 'clamp(100px,12vw,148px) 20px clamp(64px,8vw,88px)', position: 'relative', overflow: 'hidden', textAlign: 'center' }}>
        ...
        <div style={{ maxWidth:640,margin:'0 auto',position:'relative' }}>
          ...
          <Reveal delay={0.14}>
            <div className="pr-hero-btns" style={{ display:'flex',gap:10,justifyContent:'center',flexWrap:'wrap' }}>
              <Link href="/registro" className="pr-cta-btn" style={{ fontSize:14,fontWeight:700,padding:'13px 28px',borderRadius:13,background:'#5da7ec',color:'#fff',textDecoration:'none',display:'inline-flex',alignItems:'center',gap:7,boxShadow:'0 4px 20px rgba(93,167,236,0.35)' }}>
                Crear cuenta gratis
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
              </Link>
              <Link href="/contacto" className="pr-cta-ghost" style={{ fontSize:14,fontWeight:600,padding:'13px 22px',borderRadius:13,border:'1.5px solid rgba(255,255,255,0.14)',color:'rgba(255,255,255,0.7)',textDecoration:'none' }}>Hablar con ventas</Link>
            </div>
          </Reveal>
        </div>
      </section>
        ...
        <div style={{ maxWidth:1040,margin:'0 auto' }}>
          <Reveal>
            <div style={{ textAlign:'center',marginBottom:52 }}>
              ...
              <h2 style={{ fontSize:'clamp(26px,4vw,46px)',fontWeight:800,color:'#0f172a',letterSpacing:'-2px',margin:'10px 0 12px',lineHeight:1.08 }}>Elige tu pack de inicio</h2>
          ...
          {packs.length === 0 ? (
            <div style={{ textAlign:'center',padding:'48px 24px',color:'#94a3b8' }}>
              <p style={{ fontSize:15 }}>Próximamente disponibles</p>
            </div>
          ) : (
            <div className="pr-plans-grid" style={{ display:'grid',gridTemplateColumns:`repeat(${packs.length},1fr)`,gap:16,alignItems:'stretch' }}>
              {packs.map((pack, i) => {
                const color       = pack.color ?? '#5da7ec'
                ...
                const inner = (
                  <div
                    className={isFeatured ? '' : 'pr-plan-card'}
                    style={{
                  ...
                  >
                    ...
                    <div style={{ display:'flex',alignItems:'flex-end',gap:4,marginBottom:8 }}>
                      <span style={{ fontSize:'clamp(44px,5vw,56px)',fontWeight:800,color:isFeatured?'#fff':'#0f172a',letterSpacing:'-2.5px',lineHeight:1 }}>{pack.price_eur}</span>
                    ...
                    <a
                      ...
                      style={{
                        ...
                        padding:'13px',borderRadius:14,fontSize:13,fontWeight:700,textDecoration:'none',
                        background: isFeatured ? color : '#f8fafc',
                        color: isFeatured ? '#fff' : '#0f172a',
                        border: isFeatured ? 'none' : '1.5px solid #e2e8f0',
                        boxShadow: isFeatured ? `0 4px 20px ${color}50` : 'none',
                      }}
                    >
        ...
        <div style={{ maxWidth:760,margin:'0 auto' }}>
          <Reveal>
            <div style={{ textAlign:'center',marginBottom:44 }}>
              ...
              <h2 style={{ fontSize:'clamp(24px,4vw,42px)',fontWeight:800,color:'#0f172a',letterSpacing:'-2px',margin:'10px 0 12px',lineHeight:1.08 }}>¿Cuántos tokens necesitas?</h2>
            ...
            <div style={{ background:'#fff',borderRadius:24,border:'1.5px solid #f1f5f9',padding:'clamp(22px,4vw,36px)',boxShadow:'0 4px 28px rgba(0,0,0,0.05)' }}>
                ...
                <div key={i} style={{ marginBottom: i < 2 ? 28 : 0 }}>
                  <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10 }}>
                    ...
                    <div style={{ display:'flex',alignItems:'center',gap:8 }}>
                      ...
                      <span style={{ fontSize:11,color:'#94a3b8',padding:'2px 8px',background:'#f8fafc',borderRadius:20,border:'1px solid #f1f5f9',whiteSpace:'nowrap' }}>{(s.val * s.cost).toFixed(2)} tkn</span>
                    </div>
                  </div>
                  <input type="range" min={s.min} max={s.max} value={s.val} onChange={e => s.set(Number(e.target.value))} style={{ background:`linear-gradient(90deg,${s.color} ${((s.val-s.min)/(s.max-s.min))*100}%,#f1f5f9 ${((s.val-s.min)/(s.max-s.min))*100}%)` }} />
              ...
              {suggestedPack && (
                <div style={{ marginTop:12,padding:'13px 16px',background:'#eff6ff',borderRadius:13,border:'1px solid #bfdbfe',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:8 }}>
                  ...
                  <Link href="/registro" style={{ fontSize:12,fontWeight:700,color:'#5da7ec',textDecoration:'none',whiteSpace:'nowrap' }}>Empezar →</Link>
                </div>
              )}
            </div>
          </Reveal>
        ...
        <div style={{ maxWidth:780,margin:'0 auto' }}>
          <Reveal>
            <div style={{ textAlign:'center',marginBottom:44 }}>
              ...
              <h2 style={{ fontSize:'clamp(24px,4vw,42px)',fontWeight:800,color:'#0f172a',letterSpacing:'-2px',margin:'10px 0 12px',lineHeight:1.08 }}>¿Cuánto cuesta cada acción?</h2>
          ...
          <Reveal delay={0.06}>
            <div style={{ background:'#fff',borderRadius:24,border:'1.5px solid #f1f5f9',overflow:'hidden',boxShadow:'0 4px 28px rgba(0,0,0,0.04)' }}>
              {tokenCosts.map((item, i) => (
                <div key={i} className="pr-cost-row" style={{ display:'flex',alignItems:'center',justifyContent:'space-between',padding:'clamp(13px,2vw,17px) clamp(16px,3vw,24px)',borderBottom:i<tokenCosts.length-1?'1px solid #f8fafc':'none',gap:14,cursor:'default' }}>
                  ...
                  <span style={{ fontSize:13,fontWeight:800,color:item.color,flexShrink:0,background:item.bg,border:`1px solid ${item.border}`,padding:'4px 13px',borderRadius:100,whiteSpace:'nowrap' }}>{item.cost} tkn</span>
                </div>
              ))}
            </div>
          </Reveal>
        ...
        <div style={{ maxWidth:780,margin:'0 auto' }}>
          <Reveal>
            <div style={{ textAlign:'center',marginBottom:44 }}>
              ...
              <h2 style={{ fontSize:'clamp(24px,4vw,42px)',fontWeight:800,color:'#0f172a',letterSpacing:'-2px',margin:'10px 0 0',lineHeight:1.08 }}>Preguntas frecuentes</h2>
            </div>
          </Reveal>
          <Reveal delay={0.06}>
          ...
          <Reveal delay={0.1}>
            <div style={{ textAlign:'center' }}>
              ...
              <Link href="/contacto" className="pr-cta-btn" style={{ fontSize:14,fontWeight:700,padding:'12px 26px',borderRadius:12,background:'#5da7ec',color:'#fff',textDecoration:'none',display:'inline-flex',alignItems:'center',gap:7,boxShadow:'0 4px 14px rgba(93,167,236,0.3)' }}>Hablar con el equipo →</Link>
            </div>
          </Reveal>
        </div>
      </section>
      ...
      <section style={{ padding:'clamp(64px,8vw,100px) 20px',background:'linear-gradient(140deg,#050d1f,#0c1e42,#071428)',textAlign:'center',position:'relative',overflow:'hidden' }}>
        ...
        <Reveal>
          <div style={{ maxWidth:520,margin:'0 auto',position:'relative' }}>
            <div style={{ width:52,height:52,borderRadius:16,background:'linear-gradient(135deg,#5da7ec,#2563eb)',display:'flex',alignItems:'center',justifyContent:'center',margin:'0 auto 24px',boxShadow:'0 8px 24px rgba(93,167,236,0.3)',animation:'pr-float 3s ease-in-out infinite' }}>
            ...
            <h2 style={{ fontSize:'clamp(26px,4.5vw,48px)',fontWeight:800,color:'#fff',letterSpacing:'-2px',margin:'0 0 16px',lineHeight:1.08 }}>Empieza hoy sin riesgos</h2>
            <p style={{ fontSize:16,color:'rgba(255,255,255,0.45)',margin:'0 0 32px',lineHeight:1.7 }}>Crea tu cuenta gratis y recibe tokens de bienvenida para probar el sistema sin pagar nada.</p>
            <div style={{ display:'flex',gap:10,justifyContent:'center',flexWrap:'wrap',marginBottom:18 }}>
              <Link href="/registro" className="pr-cta-btn" style={{ fontSize:14,fontWeight:700,padding:'13px 30px',borderRadius:13,background:'#5da7ec',color:'#fff',textDecoration:'none',display:'inline-flex',alignItems:'center',gap:8,boxShadow:'0 4px 24px rgba(93,167,236,0.35)' }}>
                Crear cuenta gratis
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
              </Link>
              <Link href="/metodologia" className="pr-cta-ghost" style={{ fontSize:14,fontWeight:600,padding:'13px 24px',borderRadius:13,border:'1.5px solid rgba(255,255,255,0.14)',color:'rgba(255,255,255,0.7)',textDecoration:'none' }}>Ver cómo funciona</Link>
            </div>
