...
export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  if (secret !== process.env.ADMIN_SECRET) redirect('/admin/login')

  const F = 'system-ui,-apple-system,sans-serif'

  return (
    <html lang="es">
      <body style={{ margin: 0, padding: 0, fontFamily: F }}>
        <style>{`
          ...
          *, *::before, *::after { box-sizing: border-box; }
          ...
          .adm-nav-link { transition: all .15s; text-decoration: none; display: flex; align-items: center; gap: 10px; padding: 9px 12px; border-radius: 14px; width: 100%; border: none; background: transparent; cursor: pointer; text-align: left; }
          .adm-nav-link:hover { background: rgba(0,0,0,.03); }
          .adm-nav-link.active { background: var(--nav-color-bg); }
          ...
          @media(min-width:768px) {
            .adm-sidebar { transform: none !important; box-shadow: none !important; }
        ...
        <div style={{ display: 'flex', minHeight: '100vh' }}>
          {/* Sidebar */}
          <aside className="adm-sidebar" style={{ position: 'fixed', top: 0, left: 0, bottom: 0, width: 240, background: '#fff', borderRight: '1px solid #f1f5f9', display: 'flex', flexDirection: 'column', padding: '0 10px 16px', zIndex: 50, transition: 'transform .25s cubic-bezier(.32,.72,0,1)' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                    <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#2EC4B6', boxShadow: '0 0 6px rgba(46,196,182,.8)', animation: 'adm-pulse 2s infinite', display: 'inline-block' }} />
            ...
            <nav style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 2, overflowY: 'auto' }}>
              {NAV.map(n => (
                <NavLink key={n.href} href={n.href} label={n.label} color={n.color} iconPath={n.icon} />
              ))}
            </nav>

            {/* Logout */}
            <form action="/api/admin/auth" method="POST" style={{ marginTop: 8 }}>
              <input type="hidden" name="_method" value="DELETE" />
              <Link href="/api/admin/auth" style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 12px', borderRadius: 14, border: '1.5px solid #fecaca', background: '#fff', color: '#dc2626', textDecoration: 'none', fontSize: 13, fontWeight: 700, transition: 'all .15s' }}
...
function NavLink({ href, label, color, iconPath }: { href: string; label: string; color: string; iconPath: string }) {
  return (
    <Link href={href} className="adm-nav-link" style={{ '--nav-color-bg': `${color}12` } as any}>
