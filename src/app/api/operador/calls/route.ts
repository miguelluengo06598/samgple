...
export async function GET(request: NextRequest) {
  ...
  const decrypted = (data ?? []).map(call => ({
    ...call,
    orders: call.orders ? {
      ...call.orders,
      phone:     decryptDet((call.orders as any).phone),
      customers: (call.orders as any).customers
        ? decryptCustomer((call.orders as any).customers)
        : null,
    } : null,
  }))

  return NextResponse.json({ calls: decrypted })
}
...
export async function PATCH(request: NextRequest) {
  ...
  const { data: callReq } = await admin
    .from('call_requests')
    .select('id, order_id, assigned_to')
    .eq('id', id)
    .single()
      ...
      const aiRes = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: 'Convierte esta nota interna de un operador en un mensaje claro y amigable para el cliente. Español, máximo 2-3 frases, tono natural.',
            },
            {
              role: 'user',
              content: `Nota: "${admin_note}"`,
            },
          ],
        }),
      })
