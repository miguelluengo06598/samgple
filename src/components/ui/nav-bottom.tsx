...
const IcChevL   = () => <svg width="14" height="14" viewBox="0 0 24 24" {...svgBase} strokeWidth="2"><polyline points="15 18 9 12 15 6"/></svg>
const IcChevR   = () => <svg width="14" height="14" viewBox="0 0 24 24" {...svgBase} strokeWidth="2"><polyline points="9 18 15 12 9 6"/></svg>
const IcChevRSm = () => <svg width="13" height="13" viewBox="0 0 24 24" {...svgBase} strokeWidth="2"><polyline points="9 18 15 12 9 6"/></svg>
const IcChevU   = () => <svg width="12" height="12" viewBox="0 0 24 24" {...svgBase} strokeWidth="2.2"><polyline points="18 15 12 9 6 15"/></svg>
const IcChevD   = () => <svg width="12" height="12" viewBox="0 0 24 24" {...svgBase} strokeWidth="2.2"><polyline points="6 9 12 15 18 9"/></svg>
...
@keyframes nb-slidein {from{opacity:0;transform:translateY(-10px) scale(.97)}to{opacity:1;transform:none}}
@keyframes nb-fadeup  {from{opacity:0;transform:translateY(10px) scale(.97)}to{opacity:1;transform:none}}
...
@keyframes nb-live    {0%,100%{opacity:1}50%{opacity:.4}}

.nb-bar{transition:width .2s cubic-bezier(.4,0,.2,1);will-change:width}

/* ── Nav item — estilo web pública ── */
.nb-btn{
...
.nb-btn:hover{background:#f8fafc!important;border-color:#dde3ea!important;color:#0f172a!important}
...
.nb-btn-col{
  ...
  border:none;background:#fff;cursor:pointer;
  font-family:inherit;outline:none;
  border-radius:14px;border:1.5px solid #e8edf2;
  -webkit-tap-highlight-color:transparent;
  transition:all .15s;padding:10px;
}
.nb-btn-col:hover{background:#f8fafc!important;border-color:#dde3ea!important}
.nb-btn-col.nb-on{background:#fff!important;border-color:#2EC4B6!important;color:#0f766e!important;box-shadow:0 0 0 3px rgba(46,196,182,.1)}
...
.nb-qk:hover{background:#f8fafc!important;border-color:#dde3ea!important;color:#64748b!important}

/* ── Dropdown items ── */
.nb-pi{transition:background .1s,color .1s;border-radius:10px}
...
.nb-sc::-webkit-scrollbar{width:0}
...
const Tok = memo(({ col, val }: { col: boolean; val: number }) => {
  ...
  if (col) return (
    <div title={`${val} tokens`} style={{ width: 42, height: 42, borderRadius: 14, background: '#fff', border: '1.5px solid #e8edf2', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#2EC4B6', cursor: 'default', margin: '0 auto' }}>
      <IcZap />
    </div>
  )

  return (
    <div style={{ borderRadius: 16, padding: '12px 14px', background: '#fff', border: '1.5px solid #e8edf2' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
          <span style={{ width: 24, height: 24, borderRadius: 8, background: 'rgba(46,196,182,.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#2EC4B6' }}><IcZap /></span>
          <span style={{ fontSize: 11, fontWeight: 700, color: '#64748b', textTransform: 'uppercase', letterSpacing: '.06em' }}>Tokens</span>
        </div>
        <span style={{ fontSize: 15, fontWeight: 800, color: '#0f172a', letterSpacing: '-.5px' }}>{val}</span>
      </div>
...
const Prof = memo(({ col, close }: { col: boolean; close: () => void }) => {
  ...
  return (
    <div ref={ref} style={{ position: 'absolute', bottom: 'calc(100% + 8px)', left: col ? 'calc(100% + 10px)' : 0, right: col ? 'auto' : 0, minWidth: 224, background: '#fff', borderRadius: 20, boxShadow: '0 20px 60px rgba(0,0,0,.12), 0 0 0 1px rgba(0,0,0,.06)', overflow: 'hidden', animation: 'nb-fadeup .16s cubic-bezier(.22,1,.36,1)', zIndex: 200 }}>
      <div style={{ padding: '14px 14px 12px', background: '#f8fafc', borderBottom: '1px solid #f1f5f9' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 38, height: 38, borderRadius: 12, background: 'linear-gradient(135deg,#2EC4B6,#1A9E8F)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 800, color: '#fff', flexShrink: 0 }}>M</div>
      ...
      <div style={{ padding: '6px 8px 0' }}>
        <Link href="/configuracion" onClick={close} className="nb-pi"
          style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '10px 10px', color: '#374151', textDecoration: 'none', fontSize: 13, fontWeight: 500 }}>
          <span style={{ color: '#94a3b8' }}><IcCog /></span>Configuración
        </Link>
      </div>
      <div style={{ borderTop: '1px solid #f1f5f9', padding: '6px 8px 10px', marginTop: 4 }}>
        <button className="nb-pi" onClick={close}
          style={{ display: 'flex', alignItems: 'center', gap: 9, width: '100%', padding: '10px 10px', background: 'transparent', border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 500, color: '#ef4444', fontFamily: 'inherit' }}>
...
/* ─── Sidebar item — estilo tarjeta web pública ─────────────────── */
const SBtn = memo(({ href, label, sc, idx, active, col, go }: {
  ...
  if (col) return (
    <div style={{ position: 'relative' }}>
      <button className={`nb-btn-col${active ? ' nb-on' : ''}`} onClick={go}
        onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
        style={{ color: active ? '#0f766e' : '#64748b' }}>
        {IC[idx]?.(active)}
      </button>
      {hov && <Tip label={label} sc={sc} />}
    </div>
  )
  ...
  return (
    <button className={`nb-btn${active ? ' nb-on' : ''}`} onClick={go}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
        {/* Icono pequeño a la izquierda — estilo web pública */}
        <span style={{
          width: 30, height: 30, borderRadius: 9,
          background: active ? 'rgba(46,196,182,.1)' : '#f1f5f9',
          border: `1px solid ${active ? '#bbf7d0' : '#e8edf2'}`,
        ...
        <span style={{ fontSize: 14, fontWeight: active ? 700 : 500, letterSpacing: '-.1px' }}>
          {label}
        </span>
      </div>
      {/* Chevron derecho — igual que el menú móvil de la web pública */}
      <span style={{ color: active ? '#2EC4B6' : '#cbd5e1', transition: 'color .15s' }}>
        <IcChevRSm />
      </span>
    </button>
  )
    ...
    <>
      ...
      <aside className="nb-desk nb-bar"
        style={{
          ...
          background: '#f4f6f8',
          borderRight: '1.5px solid #e8edf2',
        }}>

        {/* Logo + colapsar */}
        <div style={{ height: 64, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: col ? 'center' : 'space-between', padding: col ? '0 14px' : '0 16px', borderBottom: '1.5px solid #e8edf2', background: '#fff' }}>
          {col
            ? <Link href="/pedidos" style={{ textDecoration: 'none' }}>
                <div style={{ width: 36, height: 36, borderRadius: 12, background: 'linear-gradient(135deg,#2EC4B6,#1A9E8F)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 12px rgba(46,196,182,.25)', color: '#fff' }}><IcZap /></div>
              </Link>
            : <>
                <Link href="/pedidos" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ width: 34, height: 34, borderRadius: 11, background: 'linear-gradient(135deg,#2EC4B6,#1A9E8F)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 12px rgba(46,196,182,.25)', flexShrink: 0, color: '#fff' }}><IcZap /></div>
                ...
                <button onClick={() => setCol(true)}
                  style={{ width: 30, height: 30, borderRadius: 9, border: '1.5px solid #e8edf2', background: '#f4f6f8', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#94a3b8', flexShrink: 0, transition: 'all .15s' }}
                  title="Colapsar [">
                  <IcChevL />
                </button>
              </>
        ...
        <div style={{ padding: '14px 12px 8px', flexShrink: 0 }}>
          <button className="nb-qk" onClick={() => setCmd(true)}
            style={{ padding: col ? '11px' : '9px 13px', borderRadius: 13, justifyContent: col ? 'center' : 'flex-start', gap: 9 }}>
            <IcSearch />
            {!col && (
              <>
                <span style={{ fontSize: 13, fontWeight: 500, flex: 1, textAlign: 'left', color: '#94a3b8' }}>Ir a…</span>
                <kbd style={{ fontSize: 10, padding: '2px 6px', borderRadius: 6, background: '#f4f6f8', border: '1.5px solid #e2e8f0', color: '#94a3b8', fontFamily: 'inherit' }}>⌘K</kbd>
              </>
            )}
          </button>
        </div>
        ...
        {/* Nav items */}
        <nav className="nb-sc" style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', padding: col ? '4px 10px' : '4px 12px', display: 'flex', flexDirection: 'column', gap: 6 }}>
          {LINKS.map((l, i) => (
        ...
        {/* Footer */}
        <div style={{ padding: col ? '12px 10px' : '12px', borderTop: '1.5px solid #e8edf2', display: 'flex', flexDirection: 'column', gap: 8, flexShrink: 0, background: '#fff' }}>

          {/* Live indicator */}
          {!col && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 12px', borderRadius: 13, background: '#f0fdf4', border: '1.5px solid #bbf7d0' }}>
              <span style={{ width: 7, height: 7, borderRadius: '50%', background: '#22c55e', boxShadow: '0 0 5px rgba(34,197,94,.5)', animation: 'nb-live 2s ease-in-out infinite', flexShrink: 0 }} />
          ...
          {col && (
            <button onClick={() => setCol(false)}
              style={{ width: '100%', padding: '10px', border: '1.5px solid #e8edf2', borderRadius: 13, background: '#f4f6f8', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#94a3b8', transition: 'all .15s' }}>
              <IcChevR />
            </button>
          )}
          ...
          <div style={{ position: 'relative' }}>
            ...
            <button onClick={() => setProf(o => !o)}
              style={{ display: 'flex', alignItems: 'center', gap: col ? 0 : 10, width: '100%', padding: col ? '8px' : '9px 12px', borderRadius: 13, border: '1.5px solid #e8edf2', background: prof ? '#f4f6f8' : '#fff', cursor: 'pointer', fontFamily: 'inherit', justifyContent: col ? 'center' : 'flex-start', transition: 'all .12s' }}>
              <div style={{ width: 32, height: 32, borderRadius: 10, background: 'linear-gradient(135deg,#2EC4B6,#1A9E8F)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 800, color: '#fff', flexShrink: 0 }}>M</div>
              {!col && (
                <>
